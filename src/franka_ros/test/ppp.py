...
def move():
          ...
          b1 = t1[-1]
          print('b1',b1)
          pose = [0, -0.8481508460237732, 0, -2.4425969022940386, 0, 1.5707963, 0.78539816]
          t2,j2 = mybot.ForwardKin(pose)
          print('t2', t2[-1])
          goal.trajectory.points[1]=JointTrajectoryPoint(positions=pose, velocities=[0]*7,time_from_start=rospy.Duration(3.0))
          Tjoint = np.array([[-np.sin(np.pi/4), np.cos(np.pi/4), 0, 0.2888],[np.cos(np.pi/4), np.sin(np.pi/4), 0, 0],[ 0, 0, 1, 0.5],[0, 0, 0, 1]])
          q, e = mybot.IterInvKin(pose,Tjoint)
          print(q)
          goal.trajectory.points[2]=JointTrajectoryPoint(positions=q, velocities=[0]*7,time_from_start=rospy.Duration(6.0))
